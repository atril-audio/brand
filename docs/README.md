...
El [`README`](../README.md) es el documento **vivo** que dice qué hay y cómo se consume: hace acá el
papel que el `PRD.md` hace en `pedalera`.

Los ADR que **cruzan repos** no van acá: van a
...
`DECISIONES.md` del taller, en el hub de Marco. Los archivos de acá lo citaban como fuente.
...
Lo que se decide sobre el sistema se registra **acá**; el taller queda como lo que es, el **historial
...
**ejecución sobre un sistema ya decidido**, y va directo a `specs/`. Un PRD acá se justifica cuando
...
2  hub: taller/  se explora e itera — acá no llega lo descartado
